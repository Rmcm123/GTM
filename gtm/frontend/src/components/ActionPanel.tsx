  ...
  return (
    <Panel className="grid gap-4">
      ...
      <div className="grid grid-cols-1 gap-2.5 md:grid-cols-2 xl:grid-cols-2">
        {actions.map((action) => (
          <button className="min-h-11 rounded-[7px] border border-[#cbd5e1] bg-[#f8fafc] text-[14px] font-bold text-[#1f2937] transition-colors hover:bg-[#eef4f2]" key={action} type="button">
            {action}
          </button>
        ))}
      </div>
