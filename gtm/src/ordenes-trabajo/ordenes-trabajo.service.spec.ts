...
import { OrdenTrabajo, EstadoOrdenTrabajo, EstadoPagoOrden } from './orden-trabajo.entity';
...
function crearOrdenBase(datos: Partial<OrdenTrabajo> = {}): OrdenTrabajo {
  return {
    ...
    total: 100000,
    totalPagado: 0,
    vehiculo: { año: 2020, kilometraje: 85000, marca: 'Toyota', modelo: 'Corolla', patente: 'ABCD12' },
...
function crearService(orden: OrdenTrabajo) {
  ...
  const service = new OrdenesTrabajoService(
    repositorioOrdenesTrabajo as never, {} as never, {} as never, {} as never, {} as never, {} as never, {} as never, {} as never, {} as never
  );
...
function crearServiceParaRepuestos(orden: OrdenTrabajo) {
  ...
  const manager = { getRepository: jest.fn().mockReturnValue(repositorioOrdenesTrabajo) };
  const dataSource = { transaction: jest.fn((callback) => callback(manager)) };
  const descuentosService = {
    calcularMejorDescuento: jest.fn().mockReturnValue({ motivo: 'Sin descuento', porcentaje: 0 }),
  };
  ...
  const service = new OrdenesTrabajoService(
    repositorioOrdenesTrabajo as never, {} as never, {} as never, {} as never, {} as never, {} as never,
    descuentosService as never, inventarioService as never, dataSource as never
  );
  return { descuentosService, inventarioService, repositorioOrdenesTrabajo, service };
}
...
describe('OrdenesTrabajoService', () => {
  ...
  describe('CRUD (Pruebas unitarias)', () => {
    ...
    beforeEach(async () => {
      ...
      dataSource = {
        transaction: jest.fn(async (cb) => cb({ getRepository: jest.fn().mockReturnValue(repoOrdenes) })),
      };
      ...
      const module: TestingModule = await Test.createTestingModule({
        providers: [
          ...
          { provide: DescuentosService, useValue: { calcularMejorDescuento: jest.fn().mockReturnValue({ porcentaje: 0, motivo: '' }) } },
          { provide: InventarioService, useValue: { calcularCostoRepuestosConManager: jest.fn().mockResolvedValue(0), registrarSalidasPorOrdenConManager: jest.fn() } },
  ...
  describe('Reglas de Negocio (Estados y Pagos)', () => {
    it('no permite pasar a En proceso sin pagar el adelanto requerido', async () => {
      const orden = crearOrdenBase({ adelantoRequerido: 40000, totalPagado: 20000 });
      const { service } = crearService(orden);
      await expect(service.actualizarEstado(1, { estado: EstadoOrdenTrabajo.EnProceso })).rejects.toThrow(BadRequestException);
    });

    it('permite pasar a En proceso cuando el adelanto esta pagado', async () => {
      const orden = crearOrdenBase({ adelantoRequerido: 40000, totalPagado: 40000 });
      const { repositorioOrdenesTrabajo, service } = crearService(orden);
      const respuesta = await service.actualizarEstado(1, { estado: EstadoOrdenTrabajo.EnProceso });
      expect(repositorioOrdenesTrabajo.save).toHaveBeenCalledWith(expect.objectContaining({ estado: EstadoOrdenTrabajo.EnProceso }));
      expect(respuesta.estado).toBe(EstadoOrdenTrabajo.EnProceso);
    });

    it('no permite entregar una orden con saldo pendiente', async () => {
      const orden = crearOrdenBase({ estado: EstadoOrdenTrabajo.Finalizada, saldoPendiente: 50000 });
      const { service } = crearService(orden);
      await expect(service.actualizarEstado(1, { estado: EstadoOrdenTrabajo.Entregada })).rejects.toThrow('saldo pendiente');
    });

    it('no permite entregar una orden que aun no esta finalizada', async () => {
      const orden = crearOrdenBase({ estado: EstadoOrdenTrabajo.EnProceso, saldoPendiente: 0, totalPagado: 100000 });
      const { service } = crearService(orden);
      await expect(service.actualizarEstado(1, { estado: EstadoOrdenTrabajo.Entregada })).rejects.toThrow('debe estar Finalizada');
    });

    it('permite entregar una orden finalizada y sin saldo pendiente', async () => {
      const orden = crearOrdenBase({ estado: EstadoOrdenTrabajo.Finalizada, estadoPago: EstadoPagoOrden.Pagada, saldoPendiente: 0, totalPagado: 100000 });
      const { repositorioOrdenesTrabajo, service } = crearService(orden);
      const respuesta = await service.actualizarEstado(1, { estado: EstadoOrdenTrabajo.Entregada });
    ...
    it('agrega repuestos solicitados y recalcula total y saldo de la orden', async () => {
      const orden = crearOrdenBase({ costoManoObra: 70000, costoRepuestos: 30000, saldoPendiente: 60000, subtotal: 100000, total: 100000, totalPagado: 40000 });
      const { inventarioService, repositorioOrdenesTrabajo, service } = crearServiceParaRepuestos(orden);
      const respuesta = await service.agregarRepuestos(1, { repuestos: [{ cantidad: 1, nombre: 'Filtro de aire' }] });
      expect(inventarioService.registrarSalidasPorOrdenConManager).toHaveBeenCalled();
