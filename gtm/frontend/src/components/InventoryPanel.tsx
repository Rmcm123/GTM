    ...
    <Panel>
      ...
      {alertasStockBajo.length > 0 && (
        <div className="mb-4 grid gap-2 rounded-[8px] border border-[#fed7aa] bg-[#fff7ed] p-3">
          <span className="text-[12px] font-extrabold uppercase text-[#9a3412]">
            Alerta stock bajo
          </span>
          {alertasStockBajo.map((alerta) => (
            <div
              className="flex flex-col gap-1 sm:flex-row sm:items-center sm:justify-between"
              key={`${alerta.repuestoId}-${alerta.creadoEn}`}
            >
        ...
        <div className="grid">
          {!Array.isArray(items) || items.length === 0 ? (
            ...
            items.map((item, index) => (
              <div
                className={`flex justify-between gap-3.5 py-3.5 ${index === 0 ? '' : 'border-t border-[#e5eaf0]'}`}
                key={item?.id ?? item?.name ?? index}
              >
                <div className="grid gap-0.5">
                  <strong className="text-[14px] font-bold text-[#111827]">
                    {item?.name || 'Desconocido'}
                  </strong>
                  <span className="text-[14px] text-[#64748b]">
                    {item?.category || 'Sin categoria'} · Minimo:{' '}
                    {item?.minimum || 0} · Precio:{' '}
                    {formatoMoneda.format(item?.unitPrice || 0)}
                  </span>
                </div>
                {(item?.stock || 0) < (item?.minimum || 0) ? (
                  ...
                  <span className="self-center whitespace-nowrap text-[14px] text-[#64748b]">
                    {item?.stock || 0} unidades
                  </span>
                )}
              </div>
      ...
      {tieneFormulario &&
        ...
        onRegistrarSalida && (
          <div
            id="update-stock-top"
            className="mt-5 border-t border-[#e5eaf0] pt-4"
          >
            <div className="mb-3">
              <span className="mb-1.5 inline-block text-[12px] font-bold uppercase text-[#64748b]">
                Crear repuesto
              ...
              <p className="m-0 text-[14px] text-[#64748b]">
                Ingresa los datos basicos del repuesto para agregarlo al
                inventario.
              </p>
            </div>
              ...
              <div className="grid gap-3 md:grid-cols-4">
                ...
                <label className="grid gap-1.5 text-[13px] font-bold text-[#475569]">
                  ...
                  <input
                    className={inputClass}
                    onChange={(evento) =>
                      onActualizarCampo('categoria', evento.target.value)
                    }
                ...
                <label className="grid gap-1.5 text-[13px] font-bold text-[#475569]">
                  ...
                  <input
                    className={inputClass}
                    onChange={(evento) =>
                      onActualizarCampo('minimo', evento.target.value)
                    }
                ...
                <label className="grid gap-1.5 text-[13px] font-bold text-[#475569]">
                  ...
                  <input
                    className={inputClass}
                    onChange={(evento) =>
                      onActualizarCampo('precioUnitario', evento.target.value)
                    }
            ...
            <div
              id="register-entry"
              className="mt-8 border-t border-[#e5eaf0] pt-6"
            >
              <div className="mb-3">
                <span className="mb-1.5 inline-block text-[12px] font-bold uppercase text-[#64748b]">
                  Reponer repuesto
                ...
                <p className="m-0 text-[14px] text-[#64748b]">
                  Selecciona un repuesto existente y agregale stock.
                </p>
              </div>
                ...
                <div className="grid gap-3 md:grid-cols-3">
                    ...
                    >
                      ...
                      {items.map((item) => (
                        <option
                          ...
                          value={item?.name || ''}
                        >
                          {item?.name || 'Desconocido'} ({item?.stock || 0}{' '}
                          unidades)
                        </option>
                      ))}
                    </select>
                  ...
                  <label className="grid gap-1.5 text-[13px] font-bold text-[#475569]">
                    ...
                    <input
                      className={inputClass}
                      onChange={(evento) =>
                        onActualizarCampo('cantidad', evento.target.value)
                      }
            ...
            >
              <div className="mb-3">
                ...
                <p className="m-0 text-[14px] text-[#64748b]">
                  Descuenta unidades de un repuesto por uso en ordenes de
                  trabajo.
                </p>
              </div>
          ...
          <div className="mt-2 grid">
            {Array.isArray(movements) &&
              movements.map((movement, index) => (
                <div
                  className={`flex justify-between gap-3.5 py-3 ${index === 0 ? '' : 'border-t border-[#e5eaf0]'}`}
                  key={`${movement?.item}-${movement?.date}-${movement?.type}-${index}`}
                >
                  <div>
                    <strong className="block text-[14px] text-[#111827]">
                      {movement?.item || 'Desconocido'}
                    </strong>
                  ...
                  <span
                    className={
                      movement?.type === 'Entrada'
                        ? 'text-[14px] font-bold text-[#0f6b52]'
                        : movement?.type === 'Actualizacion'
                          ? 'text-[14px] font-bold text-[#1e55a8]'
                          : 'text-[14px] font-bold text-[#9a4b00]'
                    }
                  >
